/******************************************************************************
 * GPU + Cholesky with SBC + zstd                                                  *
 * Algunas observaciones:						      *
 * - nro_size: va tener la cantidad exacta de la compresion 		      *
*******************************************************************************/
...
void GenerateBlocks(const char *tmp,long dim, long nb, int ** dev, int p, int q,int r, size_t* nro_size){
	...
	for(long i=0; i< nro_blocks;i++)
                    ...
                    device(node_local)
		{
		    ...
		    int res = world_rank%num_devices;
                    cudaSetDevice(res);
		    ...
		    FillBlock(nb, nb, i, j, dim, nb, h_uncompressed);
                    ...
                    nvcompBatchedZstdOpts_t zstd_opts;
                    memset(&zstd_opts, 0, sizeof(nvcompBatchedZstdOpts_t));
                    
                    
                    {
                        nvcomp::ZstdManager zstd_manager(
                            uncomp_chunk_size_bytes,
                            zstd_opts,
                            stream
                        );

                        nvcomp::CompressionConfig comp_config = zstd_manager.configure_compression(uncompressed_size);

                        cudaMalloc(&device_compressed_data, comp_config.max_compressed_buffer_size);
                        cudaMalloc(&device_compressed_size, sizeof(size_t));

                        zstd_manager.compress(
...
//                    printf("Ratio -> %.2f \n",(double)uncompressed_size/actual_compressed_bytes);
...
void Cholesky_Factorization(float * a,const char * tmp, long dim, long nb, int p, int q,int r,size_t* nro_size){
	...
	for (long k = 0; k < nro_blocks; k++) {
		...
		size_t uncomp_chunk_size_bytes = uncomp_chunk_size_elements * sizeof(float);
		    ...
		    nvcomp::DecompressionConfig decomp_config = zstd_manager.configure_decompression(dDiag_compressed, &Block_Diag[0]);

                    // Allocate memory for the decompressed data
                    float *dA;
                    cudaMalloc(&dA, decomp_config.decomp_data_size);

                    zstd_manager.decompress(
	    ...
	    for (long m = k + 1; m < nro_blocks; m++) {
		    ...
		    uint8_t* device_compressed_data = nullptr;
...

                    {
                        ...
                        nvcomp::DecompressionConfig decomp_configA = zstd_manager.configure_decompression(dDiag_compressed, &Block_Diag[0]);
                        float *dA;
                        ...
                        nvcomp::DecompressionConfig decomp_configB = zstd_manager.configure_decompression(dBmk_compressed, &Block_mk[0]);
                        float *dB;
	    ...
	    for (long m = k + 1; m < nro_blocks; m++) {
		    ...
		    uint8_t* device_compressed_data = nullptr;
...

                    {
                        ...
                        cudaMalloc(&dBmk_compressed, Block_mk[0]);
                        cudaMemcpyAsync(dBmk_compressed, hBmk_compressed, Block_mk[0], cudaMemcpyHostToDevice);
                        nvcomp::DecompressionConfig decomp_configA = zstd_manager.configure_decompression(dBmk_compressed, &Block_mk[0]);
                        float *dA;
                        cudaMalloc(&dA, decomp_configA.decomp_data_size);
                        zstd_manager.decompress(
                                        reinterpret_cast<uint8_t*>(dA),
                        ...
                        nvcomp::DecompressionConfig decomp_configC = zstd_manager.configure_decompression(dBmm_compressed, &Block_mm[0]);
                        float *dC;
			...
			uint8_t *hBmn_compressed;
		        ...
		        uint8_t* device_compressed_data = nullptr;
...

                        {
                            ...
                            nvcomp::DecompressionConfig decomp_configB = zstd_manager.configure_decompression(dBnk_compressed, &Block_nk[0]);
                            float *dB;
                            cudaMalloc(&dB, decomp_configB.decomp_data_size);
                            zstd_manager.decompress(
                                        reinterpret_cast<uint8_t*>(dB),
                            ...
                            nvcomp::DecompressionConfig decomp_configC = zstd_manager.configure_decompression(dBmn_compressed, &Block_mn[0]);
                            float *dC;
                            cudaMalloc(&dC, decomp_configC.decomp_data_size);
                            zstd_manager.decompress(
                                        reinterpret_cast<uint8_t*>(dC),
